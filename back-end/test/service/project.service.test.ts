...
test('given valid projects, when getAllProjects is called as user, then all projects of user are returned', async () => {
    projectDb.getAllProjectsByUserId = mockProjectsDbGetAllProjectsByUserId.mockReturnValue([project1]);
...
//     projectDb.getProjectById = jest.fn().mockReturnValue(project1);
...
//     projectDb.getProjectById = jest.fn().mockReturnValue(project1);
//     userDb.getUserById = jest.fn().mockReturnValue(user);
...
test('given valid project, when createProject is called, then project is returned', async () => {
    projectDb.getAllProjects = jest.fn().mockReturnValue([project2]);
    projectDb.createProject = jest.fn().mockReturnValue(project1);
...
test('given valid project with name already in database, when createProject is called, then error is thrown', async () => {
    projectDb.getAllProjects = jest.fn().mockReturnValue([project1]);
    projectDb.createProject = jest.fn().mockReturnValue(project1);
...
test('given valid project, when deleteProject is called as admin, then project is returned', async () => {
    projectDb.getProjectById = jest.fn().mockReturnValue(project1);
    projectDb.deleteById = jest.fn().mockReturnValue(project1);
    taskDb.getAllTasks = jest.fn().mockReturnValue([]);
    taskDb.deleteById = jest.fn().mockReturnValue(null);
...
test('given invalid project, when deleteProject is called as admin, then error is thrown', async () => {
    projectDb.getProjectById = jest.fn().mockReturnValue(undefined);
    projectDb.deleteById = jest.fn().mockReturnValue(project1);
    taskDb.getAllTasks = jest.fn().mockReturnValue([]);
    taskDb.deleteById = jest.fn().mockReturnValue(null);
